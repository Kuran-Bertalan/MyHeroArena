...
public class Character : MonoBehaviour
{
	...
	public Global.CharacterState state;
	...
	void Update()
	{
		if (state == Global.CharacterState.Dead) return;
		...
		if (isUsingSkill && skillAttackDelay < Time.time && !isMovingToTarget)
		{
			...
			{
				InstantiateParticlesOver(currentSkill.target, currentSkill.effectObject, currentSkill.effectObjectPosition);
			}
	...
	public void Damage(int attackValue, Character currentTarget)
	{
		int damage = TakeDamage(attackValue, currentArmor);
		DamageTheTarget(damage);
        if (currentTarget.tag == "Enemy")
        {
			battleUIManager.updateEnemyHealth(currentHealth, currentTarget.id);
        }
        else
        {
			Debug.Log(currentTarget.name);
			battleUIManager.updatePlayerCharactersHealth(currentHealth, currentTarget.id);
        }
	}
	...
	private void DamageTheTarget(int damage)
	{
		...
		{
			currentHealth = 0;
			state = Global.CharacterState.Dead;
			mAnimator.SetTrigger("TrDying");
			//monsterAnimator.PlayDead();
		}

		battleUIManager.DisplayDamage(damage, GetTopPosition(), Color.white);
	}


	public void DamageBySkill(Skill skill, Character target)
	{
		target.Damage(currentAttack, target);
		if (target.tag == "Enemy")
		{
			battleUIManager.updateEnemyHealth(currentAttack, currentTarget.id);
		}
		else
		{
			Debug.Log(target.name);
			battleUIManager.updatePlayerCharactersHealth(currentAttack, currentTarget.id);
		}
	}
	...
	public void InstantiateParticlesOver(Global.Targets Target, GameObject particleObject, Global.Position position)
	{
		if (Target == Global.Targets.Ally || Target == Global.Targets.Enemy)
		{
